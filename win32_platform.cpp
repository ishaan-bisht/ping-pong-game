...
LRESULT CALLBACK window_callback(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
	...
	switch (uMsg) {
		...
		case WM_SIZE: {
			...
			int render_statesize = render_state.width * render_state.height * sizeof(unsigned int);

			if (render_state.memory) VirtualFree(render_state.memory, 0, MEM_RELEASE);
			render_state.memory = VirtualAlloc(0, render_statesize, MEM_COMMIT | MEM_RESERVE, PAGE_READWRITE);
